...
class WeatherData:
	def __init__(self, day_of_week, low, high, condition_code, condition_text, precip, humidity, wind_dir, wind_speed, wind_gusts, city, sunrise, sunset, moon_phase, moon_icon, bar_read, bar_desc, uv_index, uv_text, dew_point):
		...
		self.uv_index = u""+uv_index
		self.uv_text = u""+uv_text
		self.dew_point = u""+dew_point
...
class WeatherText:
	...
	conditions_text_ru = {
		...
		"25": _(u"Холодно"),
		"26": _(u"Облачно"),
		"27": _(u"Преимущественно облачно"),
		"28": _(u"Преимущественно облачно"),
		...
		"36": _(u"Жарко"),
		"37": _(u"Локальные грозы"),
		"38": _(u"Небольшие грозы"),
		"39": _(u"Небольшие грозы"),
		"40": _(u"Небольшие грозы"),
		"41": _(u"Сильный снегопад"),
		"42": _(u"Слабый снег"),
		"43": _(u"Сильный снегопад"),
		"44": _(u"Переменная облачность"),
		"45": _(u"Ливни"),
		"46": _(u"Снегопад"),
		"47": _(u"Локальные грозы"),
		"na": _(u"N/A"),
		"-": _(u"N/A")
}
	...
	day_of_week_short_fr = {
		...
		"Sunday": _(u"Воскресенье")
	}

	day_of_week_short_ru = {
		"Today": _(u"Сег"),
		"Monday": _(u"Пон"),
		"Tuesday": _(u"Вто"),
		"Wednesday": _(u"Сре"),
	...
	def __init__(self,options):
		...
		if self.options.locale == None:
			try:
				...
				#self.locale = "ru" #uncomment this line to force Russian locale				
			except:
			...
			if startday == None: # current conditions
				...
				if datatype == "DW":
					if self.locale == "es":
						...
						if shortweekday == True:
							output = WeatherText.day_of_week_short_ru[self.current_conditions[0].day_of_week]
						else:
							output = WeatherText.day_of_week_ru[self.current_conditions[0].day_of_week]							
					else:
					...
					if self.locale == "es":
						...
						output = WeatherText.conditions_text_ru[self.current_conditions[0].condition_code]						
					else:
					...
					if self.locale == "es":
						...
						output = WeatherText.bearing_text_ru[string]
					else:
						output = string
					...
					output = self.current_conditions[0].uv_index
				elif datatype == "UT":
					output = self.current_conditions[0].uv_text
				elif datatype == "DP":
					output = self.current_conditions[0].dew_point															
				else:
					...
					for day_number in range(startday, endday+1):
						...
						if datatype == "DW":
							if self.locale == "es":
								...
								if shortweekday == True:
									output = output + self.getSpaces(spaces) + WeatherText.day_of_week_short_ru[self.night_forecast[day_number].day_of_week]
								else:
									output = output + self.getSpaces(spaces) + WeatherText.day_of_week_ru[self.night_forecast[day_number].day_of_week]
							else:
								if shortweekday == True:
							...
							if self.locale == "es":
								...
								output = output + self.getSpaces(spaces) + WeatherText.conditions_text_ru[self.night_forecast[day_number].condition_code]
							else:
							...
							if self.locale == "es":
								...
								output = output + self.getSpaces(spaces) + WeatherText.bearing_text_ru[string]
							else:
								output = output + self.getSpaces(spaces) + string
							...
							output = output + self.getSpaces(spaces) + self.night_forecast[day_number].uv_index
						elif datatype == "UT":
							output = output + self.getSpaces(spaces) + self.night_forecast[day_number].uv_text
						elif datatype == "DP":
							output = output + self.getSpaces(spaces) + self.night_forecast[day_number].dew_point								
						else:
							output = "\nERROR:Unknown data type requested"
							break
					...
					for day_number in range(startday, endday+1):
						...
						if datatype == "DW":
							if self.locale == "es":
								...
								if shortweekday == True:
									output = output + self.getSpaces(spaces) + WeatherText.day_of_week_short_ru[self.day_forecast[day_number].day_of_week]
								else:
									output = output + self.getSpaces(spaces) + WeatherText.day_of_week_ru[self.day_forecast[day_number].day_of_week]															
							else:
								if shortweekday == True:
							...
							if self.locale == "es":
								...
								output = output + self.getSpaces(spaces) + WeatherText.conditions_text_ru[self.day_forecast[day_number].condition_code]
							else:
							...
							output = output + self.getSpaces(spaces) + self.day_forecast[day_number].uv_index
						elif datatype == "UT":
							output = output + self.getSpaces(spaces) + self.day_forecast[day_number].uv_text
						elif datatype == "DP":
							output = output + self.getSpaces(spaces) + self.day_forecast[day_number].dew_point								
						else:
							output = u"\nERROR:Unknown data type requested"
							break
		...
		if self.isConnectionAvailable() == False:
 		...
 		if RefetchData == True:
			...
			try:
				...
				uv_n = current_condition_n.getElementsByTagName('uv')[0]
				uv_index_n = uv_n.getElementsByTagName('i')[0]
				uv_index = self.getText(uv_index_n.childNodes)
				uv_text_n = uv_n.getElementsByTagName('t')[0]
				uv_text = self.getText(uv_text_n.childNodes)
				dew_point_n = current_condition_n.getElementsByTagName('dewp')[0]
				dew_point = self.getText(dew_point_n.childNodes)
				current_conditions_data = WeatherData(day_of_week, current_temp_feels, current_temp, current_code, current_desc, precip, humidity, wind_direction, wind_speed, wind_gusts, city, sunrise, sunset, moon_phase, moon_icon, bar_read, bar_desc, uv_index, uv_text, dew_point)
				...
				for day in day_nodes:
					...
					day_forecast_data = WeatherData(day_of_week, low_temp, high_temp, condition_code, condition, precip, humidity, wind_direction, wind_speed, wind_gusts, city, sunrise, sunset, moon_phase, moon_icon, bar_read, bar_desc, uv_index, uv_text, dew_point)
					...
					wind_gusts_n = wind_n.getElementsByTagName('gust')[0]
					wind_gusts = self.getText(wind_gusts_n.childNodes)
					night_forecast_data = WeatherData(day_of_week, low_temp, high_temp, condition_code, condition, precip, humidity, wind_direction, wind_speed, wind_gusts, city, sunrise, sunset, moon_phase, moon_icon, bar_read, bar_desc, uv_index, uv_text, dew_point)
